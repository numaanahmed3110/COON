...
export class JavaScriptLanguageHandler implements LanguageHandler {
    ...
    private getLanguageDataPath(): string {
        const jsPath = path.resolve(__dirname, '..', '..', '..', '..', '..', 'spec', 'languages', 'javascript');
    ...
    private getComponents(): Record<string, Record<string, string>> {
        if (!this._components) {
            this._components = this.loadJsonFile<Record<string, Record<string, string>>>('components.json');
        }
     ...
     */
    createLexer(_source: string): any {
